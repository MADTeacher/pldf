...
7.5. **Запись решений в Decision Log**:
   ...
   - В разделе "Decision Log" → "История решений" (в конце файла `architecture.md`) добавь записи о:
   ...
   - Для каждого решения укажи:
     ...
     - **Связанные решения**: ссылки на решения из `tech-stack.md` и `ui-design.md`, которые повлияли на архитектурные решения
...
7.6. **Обновление project-context.md**:
   - Обнови `.pldf/project-context.md`:
     ...
     - Добавь ссылку на Decision Log в `architecture.md`
   - **ВАЖНО**: НЕ добавляй Decision Log напрямую в `project-context.md`, только ссылки

8. **Сохранение артефактов**:
   - Сохрани `.pldf/architecture.md` (с Decision Log в конце)
