...
7. **Коммит изменений в GIT**:
   ...
   - **Примечание**: Все изменения шага (код, тесты, документация, обновленный `.pldf/memory/progress.json`) должны быть включены в один коммит. Если тесты добавляются позже, создай отдельный коммит: `git commit -m "test(step-NN): добавлены тесты для шага [название]"`
