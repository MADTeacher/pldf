...
def callback(ctx: typer.Context):
    ...
    if (
        ...
        show_banner()
        console.print(
            Align.center("[dim]Run 'pldf --help' for usage information[/dim]")
        )
        console.print()
...
def init_git_repo(
    ...
    try:
        ...
        subprocess.run(
            ["git", "commit", "-m", "Initial commit from PLDF template"],
            check=True,
            capture_output=True,
            text=True,
        )
...
def init(
    ...
    if ai_assistant:
        ...
        selected_ai = select_with_arrows(
            ai_choices, "Choose your AI assistant:", "cursor-agent"
        )
    ...
    steps_lines.append(
        "   2.5 [cyan]/pldf.deploy[/] - Plan deployment (environments, CI/CD, secrets, observability)"
    )
